...
export async function POST(request: Request) {
  ...
  for (const name of [FIREBASE_SESSION_COOKIE, "kvartal_admin_session", CSRF_COOKIE]) response.cookies.set(name, "", { ...expiredAuthCookieOptions, httpOnly: name !== CSRF_COOKIE });
  return response;
}
