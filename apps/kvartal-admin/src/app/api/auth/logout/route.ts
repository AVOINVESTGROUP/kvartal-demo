...
export async function POST(request: Request) {
  ...
  const allowedOrigin = process.env.KVARTAL_ADMIN_ORIGIN ?? (process.env.NODE_ENV !== "production" ? new URL(request.url).origin : "");
  ...
  for (const name of [FIREBASE_SESSION_COOKIE, "kvartal_admin_session", CSRF_COOKIE]) response.cookies.set(name, "", { httpOnly: name !== CSRF_COOKIE, secure: true, sameSite: "strict", path: "/", maxAge: 0 });
  return response;
}
