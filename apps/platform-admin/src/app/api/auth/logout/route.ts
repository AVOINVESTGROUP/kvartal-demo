...
export async function POST(request: Request) {
  ...
  for (const name of [FIREBASE_SESSION_COOKIE, "fixer_platform_session", CSRF_COOKIE]) response.cookies.set(name, "", { httpOnly: name !== CSRF_COOKIE, secure: true, sameSite: "strict", path: "/", maxAge: 0 });
  return response;
}
