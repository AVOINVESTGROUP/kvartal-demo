...
import { CorporateSafeChallengePanel, RegistryBootstrapPanel, SafeExecutionButton, SafeOperationButton } from "./safe-operation-button";
...
async function registerContract(formData: FormData) {
  "use server";
  await activateRegistryContract({
...
async function activateRegistryContract(submission: { contractAddress: string; registryAdminSafeAddress: string; deploymentTxHash: string; abiHash: string; version: string; reason: string }) {
  "use server";
  await requirePlatformOwner();
  const result = await writeSecureActorBackendJson<{ ok: boolean; contract: { contractAddress: string } }>(process.env.PLATFORM_API_BASE_URL, "/api/v1/platform/property-identity/web3/contracts/register", "POST", submission, { "Idempotency-Key": `contract-register:${crypto.randomUUID()}` });
  revalidatePath("/property-identity/web3");
  return { ok: result.ok, contractAddress: result.contract.contractAddress };
}
...
async function submitCorporateSafeSignature(walletId: string, submission: { safeMessageHash: string; senderAddress: string; senderSignature: string }) {
  "use server";
  await requirePlatformOwner();
  const result = await writeSecureActorBackendJson<{ ok: boolean; status: string; confirmations: number; confirmationsRequired: number; productionReady?: boolean }>(process.env.PLATFORM_API_BASE_URL, `/api/v1/platform/property-identity/web3/corporate-wallets/${encodeURIComponent(walletId)}/submit-safe-message-signature`, "POST", submission, { "Idempotency-Key": `corporate-safe-signature:${crypto.randomUUID()}` });
  revalidatePath("/property-identity/web3");
  return result;
}
...
async function recordChainTransaction(formData: FormData) {
  ...
  const recorded = await writeSecureActorBackendJson<{ ok: boolean; tokenRecordId: string }>(process.env.PLATFORM_API_BASE_URL, `/api/v1/platform/property-identity/web3/token-operations/${encodeURIComponent(operationId)}/record-chain-tx`, "POST", { chainTxHash: value(formData, "chainTxHash"), registrySafeTxHash: value(formData, "registrySafeTxHash") }, { "Idempotency-Key": `record-chain-tx:${crypto.randomUUID()}` });
  await writeSecureActorBackendJson(process.env.PLATFORM_API_BASE_URL, `/api/v1/platform/property-identity/web3/tokens/${encodeURIComponent(recorded.tokenRecordId)}/reconcile`, "POST", {}, { "Idempotency-Key": `reconcile-after-record:${crypto.randomUUID()}` });
  revalidatePath("/property-identity/web3");
}
...
async function confirmSafeTransaction(operationId: string, senderAddress: string, senderSignature: string) {
  "use server";
  await requirePlatformOwner();
  const result = await writeSecureActorBackendJson<{ ok: boolean; status: string; confirmations: number; confirmationsRequired: number; safeTxHash: string }>(process.env.PLATFORM_API_BASE_URL, `/api/v1/platform/property-identity/web3/token-operations/${encodeURIComponent(operationId)}/confirm-safe-transaction`, "POST", { senderAddress, senderSignature }, { "Idempotency-Key": `safe-confirmation:${crypto.randomUUID()}` });
  revalidatePath("/property-identity/web3");
  return result;
}
...
async function recordSafeExecution(operationId: string, safeTxHash: string, chainTxHash: string) {
  "use server";
  await requirePlatformOwner();
  const recorded = await writeSecureActorBackendJson<{ ok: boolean; tokenRecordId: string }>(process.env.PLATFORM_API_BASE_URL, `/api/v1/platform/property-identity/web3/token-operations/${encodeURIComponent(operationId)}/record-chain-tx`, "POST", { registrySafeTxHash: safeTxHash, chainTxHash }, { "Idempotency-Key": `safe-executed:${crypto.randomUUID()}` });
  const reconciled = await writeSecureActorBackendJson<{ ok: boolean; reconciliationStatus: string }>(process.env.PLATFORM_API_BASE_URL, `/api/v1/platform/property-identity/web3/tokens/${encodeURIComponent(recorded.tokenRecordId)}/reconcile`, "POST", {}, { "Idempotency-Key": `reconcile-after-safe:${crypto.randomUUID()}` });
  revalidatePath("/property-identity/web3");
  return { ok: true, reconciliationStatus: reconciled.reconciliationStatus };
}
...
export default async function PropertyIdentityWeb3Page() {
  ...
  return <main className="min-h-screen bg-kv-bg p-6 text-kv-ink"><div className="mx-auto max-w-[1280px]">
    ...
    <section className="mt-6 rounded-md border border-kv-line bg-white p-4"><h2 className="font-black text-kv-navy">Привязать Corporate Safe агентства</h2><p className="mt-1 text-sm text-kv-muted">Выберите организацию и создайте EIP-712 challenge. Затем два владельца Safe подпишут его кнопкой в карточке ниже — без ручной сборки подписи и без расхода gas. Для первого запуска можно использовать уже созданный Registry/Admin Safe, если он действительно принадлежит владельцам этой организации.</p><form action={issueWalletChallenge} className="mt-3 grid gap-2 md:grid-cols-[1fr_2fr_auto]"><input type="hidden" name="chainId" value={data.chain.chainId}/><select name="organizationId" required className="min-h-11 rounded border border-kv-line px-3">{data.organizations.map(item => <option key={item.id} value={item.id}>{item.legalName}</option>)}</select><input name="walletAddress" required defaultValue={data.readiness.registryAdminSafeAddress ?? ""} placeholder="0x Corporate Safe address" className="min-h-11 rounded border border-kv-line px-3 font-mono"/><button className="rounded bg-kv-navy px-4 py-2 font-black text-white">Создать challenge</button></form></section>

    {!data.readiness.activeRegistryContract ? <section className="mt-6 rounded-md border-2 border-kv-red bg-white p-5"><h2 className="text-xl font-black text-kv-navy">Ввести Web3-реестр в работу</h2><p className="mt-1 text-sm text-kv-muted">Один управляемый процесс создаст Registry/Admin Safe с порогом 2 подписи, развернёт BEP‑721 контракт в BNB Mainnet и передаст результат API для обязательной on-chain проверки. Укажите минимум два принадлежащих владельцам платформы публичных адреса; подключённый MetaMask должен быть одним из них.</p><RegistryBootstrapPanel chainId={data.chain.chainId} writesAllowed={data.chain.writesAllowed} bytecode={registryArtifact.bytecode} abiJson={JSON.stringify(registryArtifact.abi)} activateAction={activateRegistryContract}/></section> : null}

    <section className="mt-6 rounded-md border border-kv-line bg-white p-4"><h2 className="font-black text-kv-navy">Активный Web3-реестр</h2><p className="mt-1 text-sm text-kv-muted">Платформа принимает только контракт с подтверждённой deployment-транзакцией, ожидаемым байткодом, BEP‑721 интерфейсом, всеми ролями Registry/Admin Safe и порогом 2-of-N.</p>{data.contracts.length === 0 ? <details className="mt-4"><summary className="cursor-pointer font-black text-kv-muted">Ручное восстановление после уже выполненного deploy</summary><form action={registerContract} className="mt-3 grid gap-2"><div className="grid gap-2 md:grid-cols-2"><input name="contractAddress" required placeholder="0x адрес контракта" className="min-h-11 rounded border border-kv-line px-3 font-mono"/><input name="registryAdminSafeAddress" required placeholder="0x Registry/Admin Safe" className="min-h-11 rounded border border-kv-line px-3 font-mono"/><input name="deploymentTxHash" required placeholder="0x deployment transaction hash" className="min-h-11 rounded border border-kv-line px-3 font-mono"/><input name="abiHash" required placeholder="0x SHA-256 ABI hash" className="min-h-11 rounded border border-kv-line px-3 font-mono"/><input name="version" required defaultValue="1.0.0" className="min-h-11 rounded border border-kv-line px-3"/><input name="reason" required minLength={10} defaultValue="Восстановление регистрации контракта BNB Mainnet" className="min-h-11 rounded border border-kv-line px-3"/></div><button className="justify-self-start rounded bg-kv-navy px-5 py-3 font-black text-white">Проверить и активировать существующий контракт</button></form></details> : null}<div className="mt-4 grid gap-2">{data.contracts.length === 0 ? <p className="rounded bg-amber-50 p-3 text-sm font-bold text-amber-800">Активного контракта пока нет: выпуск токенов заблокирован.</p> : data.contracts.map(contract => <div key={contract.id} className="rounded bg-kv-bg p-3 text-sm"><div className="flex flex-wrap justify-between gap-2"><b>{contract.version} · {contract.status}{contract.active ? " · ACTIVE" : ""}</b>{contract.explorerUrl ? <a href={contract.explorerUrl} target="_blank" rel="noreferrer" className="font-black text-kv-red">BscScan ↗</a> : null}</div><div className="mt-1 break-all font-mono text-xs">Contract: {contract.contractAddress}<br/>Registry Safe: {contract.registryAdminSafeAddress ?? "не зафиксирован"}<br/>Deploy tx: {contract.deploymentTxHash}<br/>Block: {contract.deploymentBlockNumber ?? "—"}</div></div>)}</div></section>

    <section className="mt-6 grid gap-4">{data.wallets.map(wallet => <article key={wallet.id} className="rounded-md border border-kv-line bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-black text-kv-navy">{wallet.organization.legalName}</h2><div className="text-xs font-mono text-kv-muted">{wallet.walletAddress}</div></div><span className="rounded-full bg-kv-bg px-3 py-1 text-xs font-black">{wallet.status} · {wallet.threshold ?? "—"}/{wallet.ownerCount ?? "—"}</span></div>{wallet.challenge ? <div className="mt-3"><CorporateSafeChallengePanel walletId={wallet.id} safeAddress={wallet.walletAddress} chainId={wallet.chainId} typedData={wallet.challenge.typedData} submitAction={submitCorporateSafeSignature}/><details className="mt-3"><summary className="cursor-pointer text-xs font-black text-kv-muted">Ручная EIP-1271 проверка</summary><div className="mt-2 text-xs font-black">Message hash: <span className="font-mono">{wallet.challenge.messageHash}</span></div><textarea readOnly value={JSON.stringify(wallet.challenge.typedData, null, 2)} className="mt-2 h-48 w-full rounded border border-kv-line bg-kv-bg p-3 font-mono text-xs"/><form action={verifyWallet} className="mt-2 flex flex-col gap-2 md:flex-row"><input type="hidden" name="walletId" value={wallet.id}/><input name="signature" required placeholder="0x агрегированная EIP-1271 подпись" className="min-h-11 flex-1 rounded border border-kv-line px-3 font-mono"/><button className="rounded bg-kv-navy px-4 py-2 font-black text-white">Проверить вручную</button></form></details></div> : null}</article>)}</section>
