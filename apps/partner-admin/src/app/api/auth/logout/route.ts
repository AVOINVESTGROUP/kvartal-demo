...
export async function POST(request: Request) {
  ...
  for (const name of [FIREBASE_SESSION_COOKIE, "partner_admin_session", "partner_admin_organization", CSRF_COOKIE]) response.cookies.set(name, "", { ...expiredAuthCookieOptions, httpOnly: name !== CSRF_COOKIE });
  return response;
}
