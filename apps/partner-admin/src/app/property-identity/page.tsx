...
type RegistryContext = {
  offices: Array<{ id: string; organizationId: string; organizationSlug: string; legalName: string; organizationName: string; city: string; country: string; defaultMarketId: string | null }>;
...
export default async function PropertyIdentityPage({ searchParams }: { searchParams?: Promise<{ submissionId?: string; officeId?: string; error?: string; message?: string }> }) {
  ...
  const configuredOrganizationSlug = process.env.PARTNER_ORGANIZATION_SLUG;
  const availableOffices = configuredOrganizationSlug
    ? context.offices.filter((item) => item.organizationSlug === configuredOrganizationSlug)
    : context.offices;
  const office = availableOffices.find((item) => item.id === params.officeId) ?? availableOffices[0];
  ...
  const defaultPolicy = policies.find((policy) => policy.jurisdiction === defaultMarket?.country && policy.subjectScope === "UNIT") ?? policies[0];
  ...
  return <main className="min-h-screen bg-kv-bg px-5 py-6 text-kv-ink">
    <div className="mx-auto max-w-[1440px] space-y-5">
      <header className="flex flex-col gap-3 rounded-md border border-kv-line bg-white p-5 lg:flex-row lg:items-center lg:justify-between">
        <div><div className="text-[12px] font-black uppercase tracking-[0.16em] text-kv-red">Property Identity Registry</div><h1 className="mt-1 text-2xl font-black text-kv-navy">Реестр физических объектов</h1><p className="mt-2 text-sm text-kv-muted">Это не заявка на рассмотрение. Вы сами создаёте черновик регистрации, проверяете объект на дубли и подтверждаете результат.</p></div>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-md border border-kv-line bg-white p-4"><div className="text-xs font-black text-kv-red">ШАГ 1</div><div className="mt-1 font-black text-kv-navy">Заполните объект</div><p className="mt-1 text-sm text-kv-muted">Укажите адрес, тип и официальный кадастровый номер.</p></div>
        <div className="rounded-md border border-kv-line bg-white p-4"><div className="text-xs font-black text-kv-red">ШАГ 2</div><div className="mt-1 font-black text-kv-navy">Проверьте дубли</div><p className="mt-1 text-sm text-kv-muted">Система найдёт уже зарегистрированный физический объект с тем же идентификатором.</p></div>
        <div className="rounded-md border border-kv-line bg-white p-4"><div className="text-xs font-black text-kv-red">ШАГ 3</div><div className="mt-1 font-black text-kv-navy">Подтвердите результат</div><p className="mt-1 text-sm text-kv-muted">Создайте новую каноническую запись либо свяжите карточку с найденной.</p></div>
      </section>

      <section className="grid gap-5 xl:grid-cols-[420px_1fr]">
        <div className="space-y-5">
          <div className="rounded-md border border-kv-line bg-white p-4">
            <h2 className="font-black text-kv-navy">Мои регистрации</h2>
            ...
            {!submissions.length ? <p className="mt-3 text-sm text-kv-muted">У вас пока нет черновиков регистрации.</p> : null}
          </div>
          <details className="rounded-md border border-kv-line bg-white" open={!selected}>
            <summary className="cursor-pointer p-4 font-black text-kv-navy">Зарегистрировать объект</summary>
            {!office ? <p className="border-t border-kv-line p-4 text-sm font-bold text-amber-700">У вашей учётной записи нет доступа к офису этой организации. Добавьте роль владельца/администратора организации либо офисную роль.</p> : !markets.length ? <p className="border-t border-kv-line p-4 text-sm font-bold text-amber-700">Реестр пока не включён для рынков этой организации. Объекты по-прежнему можно вести в обычном разделе кабинета.</p> : <form action={createSubmissionAction} className="grid gap-3 border-t border-kv-line p-4">
              ...
              <div className="grid grid-cols-2 gap-2"><label className="text-sm font-bold">Страна реестра<input name="jurisdiction" required maxLength={16} defaultValue={defaultMarket?.country} placeholder="RU / GE / AE" className="mt-1 h-10 w-full rounded border border-kv-line px-3"/></label><label className="text-sm font-bold">Что регистрируем<select name="subjectScope" defaultValue="UNIT" className="mt-1 h-10 w-full rounded border border-kv-line px-3"><option value="UNIT">Помещение / квартира</option><option value="PREMISE">Помещение</option><option value="BUILDING">Здание</option><option value="LAND_PARCEL">Земельный участок</option><option value="PROJECT">Проект</option></select></label></div>
              ...
              <div className="rounded border border-kv-line bg-kv-bg p-3"><div className="text-sm font-black text-kv-navy">Официальный идентификатор</div><p className="mt-1 text-xs text-kv-muted">Для России укажите кадастровый номер из Росреестра.</p><input name="scheme1" list="identity-schemes" required defaultValue={defaultPolicy?.identifierScheme} placeholder="Тип идентификатора" className="mt-2 h-10 w-full rounded border border-kv-line px-3"/><input name="namespace1" required defaultValue={defaultPolicy && !defaultPolicy.authorityNamespacePattern.endsWith("*") ? defaultPolicy.authorityNamespacePattern : undefined} placeholder="Орган реестра" className="mt-2 h-10 w-full rounded border border-kv-line px-3"/><input name="identifier1" required placeholder="Кадастровый номер, например 77:01:0001001:1234" className="mt-2 h-10 w-full rounded border border-kv-line px-3"/><datalist id="identity-schemes">{[...new Set(policies.map((policy) => policy.identifierScheme))].map((scheme) => <option key={scheme} value={scheme}/>)}</datalist></div>
              <button className="rounded-full bg-kv-red px-5 py-3 text-sm font-black text-white">Создать черновик регистрации</button>
            </form>}
          </details>
        </div>
        ...
        </div> : <div><h2 className="font-black text-kv-navy">Здесь появится ход регистрации</h2><p className="mt-2 text-kv-muted">Создайте черновик слева. После сохранения здесь будут кнопки проверки дублей и окончательного подтверждения.</p></div>}</div>
      </section>
    </div>
  </main>;
}
