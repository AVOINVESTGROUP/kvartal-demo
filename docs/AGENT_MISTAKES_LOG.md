...
| Hosted admin login used a popup-first Google Auth flow that stalled under the browser's Cross-Origin-Opener-Policy handling | Popup close detection depends on cross-window access that hosted browser policies can restrict even when Firebase authorized domains are correct | Use redirect-first authentication for all hosted admin apps and keep a source-contract test that rejects `signInWithPopup` |
